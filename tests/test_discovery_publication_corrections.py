...
class QuotaFailureGateway(FakeYouTubeGateway):
    ...
    def search_videos(self, access_token, q, published_after=None, limit=25, region_code=None, relevance_language=None):
...
def test_corr_pub_02_quota_after_success_retains_previous_batch(app):
    """CORR-PUB-02 — Si ocurre una falla de cuota en medio de las búsquedas, no se publica ningún resultado parcial de la categoría y el lote anterior sigue active."""
    ...
    with app.app_context():
        ...
        try:
            ...
            conn.execute("INSERT INTO category_keywords (category_id, term, polarity, weight) VALUES (1, 'retrato', 'positive', 0.9)")
            
            # Canal y video preexistente del lote anterior (refresh_run 1)
            conn.execute("INSERT INTO refresh_runs (id, status, requested_stages_json, current_stage, requested_at, counters_json, errors_json) VALUES (1, 'succeeded', '[\"discovery\"]', 'discovery', 'now', '{}', '[]')")
            conn.execute("INSERT INTO channels (id, youtube_channel_id, title, created_at, updated_at) VALUES (10, 'UC_OLD', 'Canal Viejo', 'now', 'now')")
            conn.execute("INSERT INTO videos (id, youtube_video_id, channel_id, title, published_at, duration_seconds, created_at, updated_at) VALUES (20, 'vid_old', 10, 'Video Viejo', '2026-07-20T10:00:00Z', 500, 'now', 'now')")
            
            # Candidato activo preexistente
            conn.execute("""
                INSERT INTO discovery_candidates (video_id, category_id, score, band, reasons_json, status, last_refresh_run_id, selection_rank, first_seen_at, last_seen_at)
                VALUES (20, 1, 60.0, 'related', '["Old reason"]', 'active', 1, 1, 'now', 'now')
            """)
            
            # Batch preexistente
            conn.execute("""
                INSERT INTO discovery_batches (refresh_run_id, category_id, target_total, selected_total, target_by_band_json, selected_by_band_json, shortfall_reason, generated_at)
                VALUES (1, 1, 8, 1, '{"related": 5, "adjacent": 2, "exploratory": 1}', '{"related": 1, "adjacent": 0, "exploratory": 0}', NULL, 'now')
            """)
            
            # Crear refresh run 2
            conn.execute("INSERT INTO refresh_runs (id, status, requested_stages_json, current_stage, requested_at, counters_json, errors_json) VALUES (2, 'running', '[\"discovery\"]', 'discovery', 'now', '{}', '[]')")
            conn.commit()
        finally:
            conn.close()
    ...
    try:
        ...
        old_cand = conn.execute("SELECT status FROM discovery_candidates WHERE video_id = 20 AND category_id = 1").fetchone()
        assert old_cand["status"] == "active", "Old candidate must remain active"

        # Check that no new candidate is active
        new_cands = conn.execute("SELECT status FROM discovery_candidates WHERE last_refresh_run_id = 2").fetchall()
        ...
        assert stats["categories"][1]["shortfall"] == "quota_exhausted", "Shortfall reason should be quota_exhausted"
    finally:
        conn.close()
